...
fn mosaicEffect(uv: vec2f) -> vec3<f32> {
  let mosaicScale = 8.0;
  let screen = u.screen_size;

  var uv2 = uv;
  uv2.x = floor(uv2.x * screen.x / mosaicScale) / (screen.x / mosaicScale) + (mosaicScale * 0.5) / screen.x;
  uv2.y = floor(uv2.y * screen.y / mosaicScale) / (screen.y / mosaicScale) + (mosaicScale * 0.5) / screen.y;

  return basePattern(uv2);
}
...
fn fs_main(
  ...
  let glassColor = mosaicEffect(uv) * u.color;
