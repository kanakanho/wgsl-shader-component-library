...
fn chromaticAberrationEffect(uv: vec2f) -> vec3<f32> {
  let offset = 0.01 * vec2f(cos(u.time), sin(u.time));
  let r = basePattern(uv + offset);
  let g = basePattern(uv);
  let b = basePattern(uv - offset);
...
fn fs_main(
  ...
  let glassColor = chromaticAberrationEffect(uv) * u.color;
