...
fn forstedGlassEffect(uv: vec2f) -> vec3<f32> {
  let radius_px = 6.0;
  let tap_count = 6.0;
  var acc = vec3f(0.0);
  ...
  for (var i: i32 = 0; i < 6; i++) {
    let fi = f32(i);
    let angle = 6.28318 * (fi / tap_count + rand(uv + vec2f(fi, 0.0)));
    let offset = vec2f(cos(angle), sin(angle)) * radius_px;
    let uv_jitter = uv + offset / u.screen_size;
    acc += basePattern(uv_jitter);
  }

  return acc / tap_count;
}
...
fn fs_main(
  ...
  let glassColor = forstedGlassEffect(uv) * u.color;
